...
public class LoggerUsageDetector extends Detector implements Detector.ClassScanner {
    ...
    /*Issue.create多个parameter的说明
    id - the fixed id of the issue
    briefDescription - short summary (typically 5-6 words or less), typically describing the problem rather than the fix
 ...
 这段代码中，我们定义了一个ISSUE，定义时传入的6个参数意义如下：
    ...
    Therefore, we created a LogUtils that wraps all our calls to Logger and disable them for release flavor.：对这条lint规则更详细的解释。
    Category.MESSAGES：类别。
    9：优先级，必须在1到10之间。
    Severity.ERROR：严重程度。其他可用的严重程度还有FATAL、WARNING、INFORMATIONAL、IGNORE。
    Implementation：这是连接Detector与Scope的桥梁，其中Detector的功能是寻找issue，而scope定义了在什么范围内查找issue。
                    在我们的例子中，我们需要在字节码级别分析用户有没有使用android.util.Log。
    */
    public static final Issue ISSUE = Issue.create(
            "LogUtilsNotUsed",
            "You must use our `LogUtils`",
            "Logging should be avoided in production for security and performance reasons. Therefore, we created a " +
                    "LogUtils that wraps all our calls to Logger and disable them for release flavor.",
            Category.MESSAGES,
            9,
            Severity.ERROR,
